...
const table= <HTMLDivElement>document.getElementById('Table')

// // _______CALCULATOR's variables_________

const dropDownSelection= <HTMLSelectElement>document.getElementById('DropDownSelection')
const checkBox= <HTMLInputElement>document.getElementById('check')

const companyName = <HTMLInputElement>document.getElementById('companyName')
const documentsPerWeek = <HTMLInputElement>document.getElementById('documentsPerWeek')
const pagePerDocument = <HTMLInputElement>document.getElementById('pagePerDocument')

const calculateButton=<HTMLButtonElement>document.getElementById('Calculate')

const allTheInputs :NodeListOf<HTMLInputElement> = document.querySelectorAll('input')

let recycledOptions: string[] = []

// //_________TABLE's variables________
const numberOfDocumentsPrintedPerYear = <HTMLTableRowElement>document.getElementById('NumberOfDocumentsPrintedPerYear')
const paperStockLabel= <HTMLTableRowElement>document.getElementById('paperStockLabel')
const michaelScottCostPerSheet= <HTMLTableRowElement>document.getElementById('MichaelScottCostPerSheet')
const competitorCostPerPage=<HTMLTableRowElement>document.getElementById('CompetitorCostPerPage')
const paperBrand=<HTMLTableRowElement>document.getElementById('paperBrand')
const additionalDetails= <HTMLTableRowElement>document.getElementById('additionalDetails')
const additionalDetailsRow= <HTMLTableRowElement>document.getElementById('additionalDetailsRow')

const annualCostForMikeResults= <HTMLElement>document.getElementById('annualCostForMikeResults')
const savingVsCompetitorResults= <HTMLElement>document.getElementById('savingVsCompetitorResults')

const adjustResponse= <HTMLButtonElement>document.getElementById('adjustResponse')

// ______VALIDATIONS variables______
const companyNameError = <HTMLElement>document.getElementById('companyNameError')
const documentsPerWeekError= <HTMLElement>document.getElementById('documentsPerWeekError')
const pagePerDocumentError= <HTMLElement>document.getElementById('pagePerDocumentError')
...
let validation :{
    ...
    page_per_document:number,
    paper_stock_option:string
} = {
    company_name: "",
...
// _____Typescript Object Sample for single product informations________

interface SingleProduct {
    paperName:string,
    brand:string,
    costPerSheetMichaelScott:string,
    costPerSheetCompetitor:string,
    recycled: boolean,
    additionalDetails:string 
}
//______Function to CREATE OPTIONS for the drop down MENU______________
paperStock_1.map(option=>{
    let optionSelection = <HTMLOptionElement>document.createElement('option')
...
const allThePaperStockOptions : NodeListOf<HTMLElement> =document.querySelectorAll('option')
...
Array.from(allTheInputs).map(input =>(input as HTMLInputElement).addEventListener('change',(e)=>{
...
const switchingPage = () =>{
    ...
    const paperTypeInfo : SingleProduct = paperStock_1.filter(product=>product.paperName === validation.paper_stock_option)[0]
    ...
    let mathForNumberOfDocumentPrintedPerYear : number = (validation.documents_per_week * validation.page_per_document)*52
    ...
    }else if(paperTypeInfo.additionalDetails.length === 0){ additionalDetailsRow.style.display = 'none' }
    ...
    let mathForAnnualCostForMikeResults : number = Number(mathForNumberOfDocumentPrintedPerYear) * Number(paperTypeInfo.costPerSheetMichaelScott.split(" ")[1])
    ...
    let competitorResults:number = Number(mathForNumberOfDocumentPrintedPerYear) * Number(paperTypeInfo.costPerSheetCompetitor.split(" ")[1])
